...
import type { ControlAPI, FormControl, SidebarSchema, SidebarTab, SidebarTabOptions, Subscription, TabAPI } from "./interfaces";
...
const getTabApi = (pluginId: string): TabAPI => {
  return {
    get get() { return (key: string) => getControl(pluginId, key) },
    get subscribe() { return (cb: Subscription) => subscribe(pluginId, cb) },
    get unsubscribe() { return (cb: Subscription) => unsubscribe(pluginId, cb) },
    get patchValue() { return (value: Record<string, any>) => patchValue(pluginId, value) }
  };
}

const registerTab = <T extends ProtoPlugin>(plugin: T, schema: SidebarSchema, options: SidebarTabOptions): TabAPI | void => {
...
$: {
  if (app) {
    app.api.sidebar = {
      get registerTab() { return registerTab }
    };

    app.api.events.subscribe(EVENT.KEY_SELECTED, (keyId: string) => {
      if (keyId !== null) {
        shownTabs = [...shownTabs, ...tabs.filter((item) => item.options.trigger === SIDEBAR_TAB_TRIGGER.KEY_SELECTED)];
      } else {
